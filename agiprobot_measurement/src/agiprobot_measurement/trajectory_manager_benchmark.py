...
class TrajectoryManager:
    ...
    def load_target_mesh(self, file_name, transform = np.eye(4), add_wbk_mirrors=True, remove_downside=True, remove_threshold_angle_deg=20):
        ...
        try:
            ...
            return False
        area_full = self.target_mesh.area
        ...
        area_red = self.target_mesh.area
    ...
    def generate_samples_and_views(self, sampling_density, uncertainty_threshold, orientations_around_boresight, view_tilt_mode="full", plan_path_to_check_reachability = False, minimum_trajectory_length = 25, trajectory_sample_step = 2):
        ...
        self.target_mesh.visual.face_colors = [0x00, 0x96, 0x82, 250]
        scene = [self.target_mesh]
        for i, pt in enumerate(sampled_surface_points):
            temp = trimesh.creation.icosphere(subdivisions=1, radius=0.8, color=[0xFF, 0x0, 0x0, 255])
            temp.apply_translation(pt)
            scene.append(temp)
        ...
        # Estimated as circle radius according to density.
        minimum_required_overmeasure = np.sqrt(1 / (np.pi * sampling_density))
        ...
        for (surface_point, face_index) in zip(sampled_surface_points, sampled_face_indices):
            for angle in result_dict["12"]["angles"]:
                ...
                if self.process_view(new_view, ik_service, uncertainty_threshold, minimum_required_overmeasure):
                    result_dict["12"]["valid_views_none"].append(new_view)
                    if angle in result_dict["9"]["angles"]:
                        result_dict["9"]["valid_views_none"].append(new_view)
                    if angle in result_dict["6"]["angles"]:
                        result_dict["6"]["valid_views_none"].append(new_view)
                    if angle in result_dict["3"]["angles"]:
                        result_dict["3"]["valid_views_none"].append(new_view)
                stop_time = time.time()
                result_dict["12"]["time_gen_and_ana_none"] += stop_time - processing_timer
                if angle in result_dict["9"]["angles"]:
                    result_dict["9"]["time_gen_and_ana_none"] += stop_time - processing_timer
                if angle in result_dict["6"]["angles"]:
                    result_dict["6"]["time_gen_and_ana_none"] += stop_time - processing_timer
                if angle in result_dict["3"]["angles"]:
                    result_dict["3"]["time_gen_and_ana_none"] += stop_time - processing_timer
                ...
                for angle_config in itertools.product([0, self.sensor_model.get_median_deviation_angle(), -1 * self.sensor_model.get_median_deviation_angle()], repeat=2):
                    ...
                    if self.process_view(new_view, ik_service, uncertainty_threshold, minimum_required_overmeasure):
                        result_dict["12"]["valid_views_full"].append(new_view)
                        if angle in result_dict["9"]["angles"]:
                            result_dict["9"]["valid_views_full"].append(new_view)
                        if angle in result_dict["6"]["angles"]:
                            result_dict["6"]["valid_views_full"].append(new_view)
                        if angle in result_dict["3"]["angles"]:
                            result_dict["3"]["valid_views_full"].append(new_view)
                    stop_time = time.time()
                    result_dict["12"]["time_gen_and_ana_full"] += stop_time - processing_timer
                    if angle in result_dict["9"]["angles"]:
                        result_dict["9"]["time_gen_and_ana_full"] += stop_time - processing_timer
                    if angle in result_dict["6"]["angles"]:
                        result_dict["6"]["time_gen_and_ana_full"] += stop_time - processing_timer
                    if angle in result_dict["3"]["angles"]:
                        result_dict["3"]["time_gen_and_ana_full"] += stop_time - processing_timer
                processed += 1
                print("Processed view {} of {} ({} %, X of them are usable for measurement)".format(
                    processed, 
                    len(sampled_surface_points) * 12, 
                    np.round(100.0 * processed / (len(sampled_surface_points) * 12), 2)
                ))
        ...
        return sampled_surface_points, result_dict
        ...
        if solver_type == "greedy":
            ...
            while len(provided_views) > 0:
                ...
                if measured_surface_point_indices == measurable_surface_point_indices:
                    ...
                    return len(measurable_surface_point_indices), used_views
            ...
            ip_problem.solve(pulp.GLPK_CMD(timeLimit=1801))
            ...
            return len(measurable_surface_point_indices), output           
...
if __name__ == "__main__":
    ...
    # os.path.join(RosPack().get_path("agiprobot_measurement"), "benchmark_meshes/objectA_wuerfel.stl"),
    # os.path.join(RosPack().get_path("agiprobot_measurement"), "benchmark_meshes/objectB_zylinder.stl"),
    # os.path.join(RosPack().get_path("agiprobot_measurement"), "benchmark_meshes/objectC_motor.stl"),
    ...
    total_result = {}
    area_full, area_red = tm.load_target_mesh(os.path.join(RosPack().get_path("agiprobot_measurement"), "benchmark_meshes/{}.stl".format(object_name)), trimesh.transformations.translation_matrix(OFFSET))
    ...
    for density in densities:
        ...
        for result in results.keys():
            partial_result.update(
                {str(result): {
                    "full": {"valid_views":len(results[result]["valid_views_full"]), "time_generation_analysis": results[result]["time_gen_and_ana_full"], "coverable_surface_points":0, "selection_results":{}}, 
                    "none": {"valid_views":len(results[result]["valid_views_none"]), "time_generation_analysis": results[result]["time_gen_and_ana_none"], "coverable_surface_points":0, "selection_results":{}}
                    }})
            for solver in solvers:
                ...
                try:
                    ...
                    partial_result[str(result)]["none"]["selection_results"].update({solver:"Timeout exceeded (30min)"})
                ...
                for view in views_ordered:
                    exec_plan.append(view.get_trajectory_to_view())
                    exec_plan.append(view.get_trajectory_for_measurement())
                tm.store_execution_plan(
                    os.path.join(RosPack().get_path("agiprobot_measurement"), "benchmark_results/execution_plan_{}_rho_{}_solver_{}_angels_around_boresight_{}.yaml".format(object_name, str(density)[2:], solver, result)),
                    exec_plan
                    )
                start_time = time.time()
                signal.alarm(1800)
    ...
    print(total_result)
    with open(os.path.join(RosPack().get_path("agiprobot_measurement"), "benchmark_results/{}.yaml".format(object_name)), "w") as ofile:
        yaml.safe_dump(total_result, ofile, default_flow_style=False)
