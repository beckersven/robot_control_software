...
class TrajectoryManager:
    ...
    def generate_samples_and_viewpoints(self, sampling_density, uncertainty_threshold, orientations_around_boresight, output_set, viewpoint_tilt_mode="full", plan_path_to_check_reachability = False, minimum_trajectory_length = 25, trajectory_sample_step = 2, static_overmeasure_mm=0):
        """
        ...
        minimum_required_overmeasure = np.sqrt(1 / (np.pi * sampling_density)) + static_overmeasure_mm
            ...
            def evaluate(self, query, index):
                if not query:
                    ...
                    try:
                        ...
                        return None
        ac = AutoCompleter()
        if click.confirm(click.style("Load trajecotry from file?", fg="cyan"), default=False):
            try:
                path_files = [filename for filename in os.listdir(os.path.join(rospkg.RosPack().get_path("agiprobot_measurement"), "trajectories")) if (filename[-4:] == ".yaml")]
            except:
            ...
            Theta = click.prompt(click.style("Set Theta in degree", fg="cyan"), type=float, default=20.0)
            ...
            if self.load_target_mesh(os.path.join(rospkg.RosPack().get_path("agiprobot_measurement"), "workpieces/{}".format(cad_file)), transform=transform, add_wbk_mirrors=wbk_mirrors, remove_downside=True, remove_threshold_angle_deg=Theta):
                ...
                return False
            rho_sampling = click.prompt(click.style("Set sampling-density in points per square-millimeters", fg="cyan"), type=float, default=0.01)
            ...
            if click.confirm(click.style("Save contructed trajectory afterwards?", fg="cyan"), default=True):
                path_name = click.prompt(click.style("Name of path in agiprobot_measurement/trajectory", fg="cyan"), type=str, default="XXX.yaml")
            else:
                path_name = None
            ...
            if path_name is not None:
                self.store_execution_plan(os.path.join(rospkg.RosPack().get_path("agiprobot_measurement"), "trajectories/{}".format(path_name)), full_trajectory, metadata={"Time": time.ctime(time.time())})
        if click.confirm(click.style("\nFull trajectory available - Execute?", fg="magenta", bold=True), default=rospy.get_param("/use_sim_time")):
            
            with click.progressbar(length=2*len(full_trajectory)) as bar:
                bar.render_progress()
