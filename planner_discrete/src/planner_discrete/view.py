...
class View:
    ...
    def __init__(self, surface_point, surface_normal, angle, standoff_distance, tilt_theta = 0, tilt_gamma = 0):
        """
        Creates view center pose based on geometric specifications (see below). It does not assign execution- or metrology-relevant attributes to the object.
        The pose is composed by moving from surface_point in surface_normal-direction standoff_distance and generate a frame with the z-axis facing
        towards the surface point (i.e. = -1 * surface_normal) and the x-axis 

        :param surface_point: Point on the targets mesh surface point this View is based on
    ...
    def get_surface_point(self, as_matrix=False):
        """
        Gets the position of the surface point this view is based on. This is not the same as the view_center-point.
